...
def home(request):
	ins_det = institute_details.objects.all()
	if ins_det.count() == 0:
...
def doRegistration(request):
	...
	# inst_obj = institute_details.objects.all().first()
	...
	if user_type == "staff":
		Staffs.objects.update_or_create(admin=user_obj,defaults={'name':first_name+" "+last_name})
	elif user_type == "student":
		Students.objects.update_or_create(admin=user_obj)
		print("boom")
	elif user_type == "hod":
		AdminHOD.objects.update_or_create(admin=user_obj)
...
def doInstReg(request):
	...
	if not name or not mail_prefix or not institute_type or not state:
		messages.error(request, "Please fill all details!!")
		return redirect("inreg")
	
	institute = institute_details.objects.update_or_create(
	name = name,
